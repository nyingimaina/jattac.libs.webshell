...
> {
  ...
  updateBackgroundColor = () => {
    ...
    this.setState({
      backgroundColor:
        bgColor === "rgba(0, 0, 0, 0)" ? "rgba(255, 255, 255, 0.9)" : bgColor,
      iconColor: color,
    });
  };
  ...
  filterItems(items: MenuItem<TId>[], searchQuery: string): MenuItem<TId>[] {
    ...
    return items.reduce<MenuItem<TId>[]>((acc, item) => {
      ...
      const itemMatches = this.getSearchKey(item)
        .toLowerCase()
        .includes(lowerQuery);

      // Filter the children if they exist
      const filteredChildren = item.children
        ? this.filterItems(item.children, searchQuery)
        : [];
  ...
  renderMenuItems(items?: MenuItem<TId>[]) {
    ...
    const filteredItems = items.filter((item) => {
      const result =
        this.getSearchKey(item)
          .toLowerCase()
          .includes(searchQuery.toLowerCase()) ||
        (item.children &&
          item.children.some((child) =>
            this.getSearchKey(child)
              .toLowerCase()
              .includes(searchQuery.toLowerCase())
          ));
      return result;
    ...
    return filteredItems.map((item, index) => {
      if (this.knownIds.some((a) => a === item.id)) {
        throw new Error(
          `Id ${item.id} has been used before in the menu.\nIds must be unique`
        );
      } else {
      ...
      return (
        <li key={item.id + ""} className={`${styles.menuItem}}`}>
          <div
            className={`${isHovered ? styles.hoveredItem : ""} ${styles.thing}`}
            onClick={() =>
              this.handleMenuItemClick(item.id, hasChildren, index)
            }
    ...
    return (
      <div className={styles.hamburgerMenu}>
        ...
        >
          {isOpen ? (
            <div
              className={`${styles.backArrow} ${styles.hamburgerIcon}`}
          ...
          {isOpen && (
            <li className={styles.menuItem}>
              <div className={styles.searchBar}>
                ...
                <input
                  type="text"
                  ref={this.searchInputRef} // Attach ref to input for focusing
